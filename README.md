...
Staging environment uses a copy of the production db, the same environment and build. Deployment is fully automated, eliminating the risk of manual errors. We don't change the code between staging and production, only environment variables are modified. If we change the code, it is no longer the same app
